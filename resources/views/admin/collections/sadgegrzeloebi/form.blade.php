...
<div class="form-group{{($error = $errors->first('tags')) ? ' validate-has-error' : '' }}">
    <label class="col-sm-2 control-label required">Tags:</label>
    <div class="col-sm-10">
        <div class="input-group">
            <span class="input-group-addon"><i class="fa fa-link"></i></span>
            {!! Form::text('tags', null, [
                'id' => 'tags' . $current->language,
                'class' => 'form-control',
                'data-lang' => 1
            ]) !!}
        </div>
...
<div class="form-group{{($error = $errors->first('categories')) ? ' validate-has-error' : '' }}">
    <label class="col-sm-2 control-label required">Categories:</label>
    <div class="col-sm-10">
        <div class="input-group">
            <span class="input-group-addon"><i class="fa fa-link"></i></span>

            {!! Form::select('categories[]', \Models\Category::where('visible', 1)->joinlanguage()->get()->pluck('title', 'id')->toArray(), null, [
                'id' => 'categories' . $current->language,
                'class' => 'form-control',
                'data-lang' => 1,
                'multiple' => true
            ]) !!}
...
<div class="form-group">
    <div class="col-sm-10 btn-action pull-right">
        ...
        <a href="{{ cms_route('sadgegrzeloebi.index', [$current->collection_id]) }}" class="btn btn-blue btn-icon-standalone" title="{{ trans('general.back') }}">
