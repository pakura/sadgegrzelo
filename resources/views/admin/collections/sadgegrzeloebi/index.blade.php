    ...
    <div class="col-md-9 pull-right has-sidebar">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <a href="{{ cms_route('sadgegrzeloebi.create', [$parent->id]) }}" class="btn btn-secondary btn-icon-standalone">
                    <i class="{{$icon}}"></i>
                        ...
                        <li id="item{{ $item->id }}" class="item" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                            <div class="uk-nestable-item clearfix">
                                <div class="row">
                                    ...
                                    @endif
                                        <div class="list-label"><a href="{{ $editUrl = cms_route('sadgegrzeloebi.edit', [$parent->id, $item->id]) }}">{{ $item->title }}</a></div>
                                    </div>
                                    <div class="col-sm-5 col-xs-2">
                                        <div class="btn-action togglable pull-right">
                                            ...
                                            {!! Form::open(['method' => 'post', 'url' => cms_route('sadgegrzeloebi.visibility', [$item->id]), 'class' => 'visibility', 'id' => 'visibility' . $item->id]) !!}
                                            ...
                                            {!! Form::close() !!}
                                            <a href="{{ cms_route('files.index', ['sadgegrzeloebi', $item->id]) }}" class="btn btn-{{$item->has_file ? 'turquoise' : 'white'}}" title="{{trans('general.files')}}">
                                                <span class="{{icon_type('files')}}"></span>
                                            </a>
                                            <a href="{{ $editUrl }}" class="btn btn-orange" title="{{trans('general.edit')}}">
                                                <span class="fa fa-edit"></span>
                                            </a>
                                            {!! Form::open(['method' => 'delete', 'url' => cms_route('sadgegrzeloebi.destroy', [$parent->id, $item->id]), 'class' => 'form-delete']) !!}
...
@push('body.bottom')
@include('admin._scripts.transfer', ['route' => cms_route('sadgegrzeloebi.transfer', [$parent->id]), 'column' => 'collection_id', 'list' => $parentSimilar, 'parentId' => $parent->id])
<script type="text/javascript">
$(function() {
@if ($parent->admin_order_by == 'position')
    positionable('{{ cms_route('sadgegrzeloebi.updatePosition') }}', '{{$parent->admin_sort}}', '{{request('page', 1)}}', '{{$items->hasMorePages()}}');
@endif
});
</script>
