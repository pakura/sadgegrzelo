...
@endif
    ...
    <ul class="nav nav-tabs col-xs-4 right-aligned">
        <li>
            <a href="{{cms_route('files.index', ['categories', $current->id])}}">
...
<div class="panel panel-default">
    <div class="panel-heading">
        ...
        <a href="{{cms_route('categories.create', [$current->collection_id])}}" class="pull-right padr">Add more</a>
    </div>
    <div class="panel-body">
        ...
        @foreach ($items as $current)
            <div class="tab-pane{{language() != $current->language ? '' : ' active'}}" id="item-{{$current->language}}">
                {!! Form::model($current, [
                    'method'    => 'put',
                    'url'       => cms_route('categories.update', [$current->collection_id, $current->id], is_multilanguage() ? $current->language : null),
